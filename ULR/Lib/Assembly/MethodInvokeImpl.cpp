...
namespace ULR
{
	// const bool big_endian = htons(1) == 1;
	
	Resolver::ULRAPIImpl* internal_api;

	// TODO: XMM ARG PASSING
	...
	/* This internal invoke method assumes correct types and length for `args`. 
		...
		for (size_t i = 0; i < lclsig.size()-1; i++)
		{
			...
			{
				if ((IsFriendlyStructSize(lclsig[i])) && !(!is_static && i == 0))
				{
		...
		{
			if (IsFriendlyStructSize(rettype))
			{
